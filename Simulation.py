...
from lib.DiscountedUCBBandit import DiscountedUCBBandit
...
class simulateOnlineData(object):
	...
	def getReward(self, user, pickedArticle, iter):
		if iter < self.testing_iterations/2:
			if pickedArticle.id == 0:
				return 1
			else:
				return 0
		else:
			if pickedArticle.id == 1:
				return 1
			else:
				return 0

	def GetOptimalReward(self, user, articlePool, iter):
		maxReward = float('-inf')
		maxx = None
		for x in articlePool:	 
			reward = self.getReward(user, x, iter)
			if reward > maxReward:
				maxReward = reward
				maxx = x
	...
	def runAlgorithms(self, algorithms):
		...
		for iter_ in range(self.testing_iterations):
			...
			for u in self.users:
				...
				OptimalReward, OptimalArticle = self.GetOptimalReward(u, self.articlePool, iter_)
				OptimalReward += noise

				for alg_name, alg in algorithms.items():
					pickedArticle = alg.decide(self.articlePool, u.id)
					reward = self.getReward(u, pickedArticle, iter_) + noise
...
if __name__ == '__main__':
	...
	algorithms['DiscountedUCBBandit'] = DiscountedUCBBandit(num_arm=n_articles, alpha=0.5, gamma=0.9)
